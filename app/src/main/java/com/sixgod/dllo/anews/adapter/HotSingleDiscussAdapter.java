 ...
 */
public class HotSingleDiscussAdapter extends BaseRecyclerAdapter<SingleDiscussData.DataEntity.HotListEntity> {
    ...
    public HotSingleDiscussAdapter(RecyclerView recyclerView, LoadListener listener, ArrayList<SingleDiscussData.DataEntity.HotListEntity> data) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position)==CONTENT_MODE||getItemViewType(position)==TOP_MODE){
            ...
            if (data.size()>0)
            {
                ...
                Toast.makeText(context, "页面暂时不能显示,稍后再试", Toast.LENGTH_SHORT).show();
